...
export default function AddCurriculumModal({ isOpen, onClose, onSaved, initialCourse }) {
  ...
  async function handleSave() {
    if (!initialCourse || !form.subject_code) return
    setLoading(true)
    try {
    ...
    >
      <div className="modal-body">
        ...
        <FormGroup label="Major">
          <MajorSelect program={initialCourse} value={form.major} onChange={(e) => set('major', e.target.value)} />
        </FormGroup>
        ...
        <FormGroup label="Subject Name">
          <FormInput value={form.subject_name} onChange={(e) => set('subject_name', e.target.value)} placeholder="e.g. Calculus I" />
        </FormGroup>
        <FormGroup label="Units">
          <FormInput type="number" value={form.units} onChange={(e) => set('units', e.target.value)} min="1" max="6" />
        </FormGroup>
      </div>
